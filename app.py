...
if os.path.exists('models'):
    ...
    for filename in required:
        ...
        if os.path.exists(filepath):
            ...
            print(f"  [OK] {filename} exists (size: {size} bytes)")
            # Try to load scaler specifically
            if filename == 'scaler.pkl':
                try:
                    test_load = joblib.load(filepath)
                    print(f"    [OK] scaler loaded successfully: {type(test_load)}")
                except Exception as e:
                    print(f"    [ERR] Failed to load scaler: {e}")
        else:
...
def predict(state):
    ...
    try:
        ...
        # ---- SHAP explanation ----
        try:
            # Recreate the processed input for SHAP
            X = preprocess_input(raw, state.selected_model)
            # Choose explainer based on model type
            if "Logistic" in state.selected_model:
                explainer = shap.LinearExplainer(active_model, X, feature_perturbation="interventional")
            else:
            ...
            plt.barh(np.arange(len(sorted_idx)), importance[sorted_idx][::-1])
            plt.yticks(np.arange(len(sorted_idx)), [X.columns[i] for i in sorted_idx][::-1])
            ...
            buf.seek(0)
            state.shap_chart = "data:image/png;base64," + base64.b64encode(buf.read()).decode()
            state.show_shap = True
        except Exception as e:
            # If SHAP computation fails, keep chart hidden
            state.shap_chart = ""
            state.show_shap = False
...
def toggle_shap(state):
    state.show_shap = not state.show_shap
...
AI RECOMMENDATIONS
<|{show_shap}|toggle|label=Show SHAP Explanation|>
<|{shap_chart}|image|class_name=shap-chart|>
