...
The dashboard now includes SHAP (SHapley Additive exPlanations) visualisations for each prediction. After a loan is analysed, the model computes SHAP values for the top 10 most influential features and displays them as a bar‑chart image. This helps users understand **why** a particular default probability was assigned and which factors contributed most to the risk score.
